...
import { RefreshCw, ArrowRight, Home, Trophy } from 'lucide-react';
import {
	ColorPicker,
...
import pantoneColors from '@/lib/pantone-colors.json';

const TOTAL_ROUNDS = 5;

// Type definition for Pantone color from JSON
interface PantoneColor {
	name: string;
	hex: string;
}

// Convert the JSON object to an array for easier random selection
const PANTONE_LIST: PantoneColor[] = Object.values(pantoneColors);

// Calculate score based on distance between two colors
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const calculateScore = (target: any, guess: any) => {
	// Using RGB euclidean distance for simplicity
...
export default function GuessTheColor() {
	...
	const [targetColorData, setTargetColorData] = useState<PantoneColor | null>(null);
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	...
	const [history, setHistory] = useState<Array<{ round: number; targetHex: string; targetName: string; guessHex: string; score: number }>>([]);

	// Pick a random color
	const pickRandomColor = () => {
		const randomIndex = Math.floor(Math.random() * PANTONE_LIST.length);
		return PANTONE_LIST[randomIndex];
	};

	useEffect(() => {
		setMounted(true);
		setTargetColorData(pickRandomColor());
		setGuessColor(Color('#ffffff'));
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, []);

	const handleGuess = () => {
		if (!targetColorData || !guessColor) return;

		const target = Color(`#${targetColorData.hex}`);
		const score = calculateScore(target, guessColor);
		...
		setHistory((prev) => [
			...
			{
				round,
				targetHex: `#${targetColorData.hex}`,
				targetName: targetColorData.name,
				guessHex: guessColor.hex(),
				score,
			},
		]);
	...
	// Format pantone name to be more readable (replace dashes with spaces and capitalize)
	const formatColorName = (name: string) => {
		return name.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
	};

	if (!mounted || !targetColorData || !guessColor) {
					...
					{gameState === 'playing' && (
						...
						>
							...
							<div className="flex flex-col items-center gap-8 w-full max-w-sm text-center">
								...
								>
									{/* Glow */}
									<div className="absolute -inset-8 rounded-[3rem] blur-3xl opacity-10 bg-primary transition-opacity duration-1000" />

									<div className="relative bg-card border border-white/10 rounded-[2.5rem] p-10 md:p-14 shadow-2xl flex flex-col items-center justify-center gap-4 min-h-[20rem]">
										<span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Target Color</span>
										<h1 className="text-4xl md:text-5xl font-black tracking-tight leading-tight text-white break-words w-full">
											{formatColorName(targetColorData.name)}
										</h1>
								...
								<div className="text-center space-y-2">
									<p className="text-muted-foreground text-sm uppercase tracking-widest font-medium">Use the picker to guess the color</p>
								</div>
							</div>
							...
							<div className="w-full grid grid-cols-2 gap-4 mb-12 h-64 md:h-80">
								...
								>
									<div className="absolute inset-x-0 bottom-0 p-6 bg-gradient-to-t from-black/50 to-transparent">
										...
										<p className="text-white/80 font-mono text-xs opacity-80 mb-1">{formatColorName(targetColorData.name)}</p>
										<p className="text-white/60 font-mono text-[10px] opacity-60">#{targetColorData.hex}</p>
									</div>
								</motion.div>
												...
												{history.map((h, i) => (
													...
													>
														...
														<div className="flex items-center gap-1 flex-1">
															<div className="w-6 h-6 rounded-full border border-white/10" style={{ backgroundColor: h.targetHex }} />
															<div className="h-px w-8 bg-white/10" />
															<div className="w-6 h-6 rounded-full border border-white/10" style={{ backgroundColor: h.guessHex }} />
														</div>
														<div className="font-mono text-xs text-white/50 text-right mr-4 hidden sm:block">
															{formatColorName(h.targetName)}
														</div>
								...
								return (
									<div
										...
										className={`
				                                        ...
				                                        ${isCurrent ? 'border-primary/50 bg-primary/5 shadow-[0_0_15px_-5px_rgba(217,70,239,0.3)] scale-105 z-10' : ''}
										...
										{roundData ? (
											<>
												<div className="flex items-center gap-1">
													<div className="w-2 h-2 rounded-full border border-white/20" style={{ backgroundColor: roundData.targetHex }} />
													<div className="w-2 h-2 rounded-full border border-white/20" style={{ backgroundColor: roundData.guessHex }} />
												</div>
